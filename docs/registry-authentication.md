...
**Requires Host Configuration** - The docker config lives on the host that runs the SOCI snapshotter. This add extra burden for Kubernetes where every node needs to be configured out-of-band to have access to credentials.


## Kubernetes CRI Credentials

In this mode, the SOCI snapshotter acts as an ImageSerivce proxy between the kubelet and containerd. When the kubelet pulls an image, it first calls the SOCI snapshotter. The SOCI snapshotter caches the credentials in the request and then forwards the request to containerd. When containerd calls back to the SOCI snapshotter to create a snapshot, the snapshotter loads the cached credentials to authenticate with the registry.
...
2. Containerd calls the SOCI snapshotter to prepare a snapshot
3. The SOCI snapshotter reads the kubeconfig to 
...
The SOCI CLI will use an explicit paramter if provided, then attempt to load credentials from docker config.
