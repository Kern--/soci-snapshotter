...
func newNode(layerDgst digest.Digest, r reader.Reader, blob remote.Blob, baseInode uint32, opaque OverlayOpaqueType, logFSOperations bool, opCounter *FuseOperationCounter, idMapper idtools.IdentityMapping) (fusefs.InodeEmbedder, error) {
...
type node struct {
	...
	id       uint32
	attr     metadata.Attr
	idMapper idtools.IdentityMapping
...
func (n *node) entryToAttr(ino uint64, e metadata.Attr, out *fuse.Attr) fusefs.StableAttr {
	...
	mappedId, _ := n.idMapper.ToHost(idtools.Identity{UID: int(e.UID), GID: int(e.GID), SID: ""})
	out.Owner = fuse.Owner{Uid: uint32(mappedId.UID), Gid: uint32(mappedId.GID)}
